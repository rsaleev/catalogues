...
CREATE TABLE public.ref_evaluation_form (
	id int4 NOT NULL GENERATED BY DEFAULT AS IDENTITY,
	title varchar(255) NOT NULL,
	regex varchar(255) NOT NULL,
	code int4 NOT NULL,
	uid uuid NOT NULL,
	CONSTRAINT ref_evaluation_form_pk PRIMARY KEY (id)
);
CREATE INDEX ref_evaluation_form_id_idx ON public.ref_evaluation_form USING btree (id);


INSERT INTO public.ref_evaluation_form (title,regex,code,uid) VALUES ('Государственный контроль (надзор)','^.*гос[а-я]+.*конт[а-я]+.*надзор.*$',0,'db6ba221-930b-4eef-a3de-eeacb7fa065c');
